...
class LambdaLayer_noBN(nn.Module):
    def __init__(self, lambd):
        super(LambdaLayer_noBN, self).__init__()
        self.lambd = lambd
...
class BasicBlock_noBN(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, option='A'):
        ...
        self.conv1 = nn.Conv2d(in_planes, planes, kernel_size=3, stride=stride, padding=1, bias=False)
        self.bn1 = _batchnorm_to_groupnorm(nn.BatchNorm2d(planes))
        self.conv2 = nn.Conv2d(planes, planes, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn2 = _batchnorm_to_groupnorm(nn.BatchNorm2d(planes))
        ...
        if stride != 1 or in_planes != planes:
            if option == 'A':
                ...
                For CIFAR10 ResNet paper uses option A.
                """
                self.shortcut = LambdaLayer_noBN(lambda x:
                                            F.pad(x[:, :, ::2, ::2], (0, 0, 0, 0, planes//4, planes//4), "constant", 0))
            elif option == 'B':
                self.shortcut = nn.Sequential(
                     nn.Conv2d(in_planes, self.expansion * planes, kernel_size=1, stride=stride, bias=False),
                     _batchnorm_to_groupnorm(nn.BatchNorm2d(self.expansion * planes))
                )
...
class ResNetNoBatchNorm(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10):
        ...
        self.conv1 = nn.Conv2d(3, 16, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn1 = _batchnorm_to_groupnorm(nn.BatchNorm2d(16))
        self.layer1 = self._make_layer(block, 16, num_blocks[0], stride=1)
        self.layer2 = self._make_layer(block, 32, num_blocks[1], stride=2)
        self.layer3 = self._make_layer(block, 64, num_blocks[2], stride=2)
        self.linear = nn.Linear(64, num_classes)
...
def resnet32_nobn(num_class=10):
    return ResNetNoBatchNorm(BasicBlock_noBN, [5, 5, 5], num_classes=num_class)
